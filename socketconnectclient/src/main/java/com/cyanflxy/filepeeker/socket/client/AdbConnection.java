...
public class AdbConnection {
    ...
    public AdbConnection(String pkgName) {
        this.pkgName = pkgName;
    }

    public Socket connect() {
        int port = ConnectionUtils.getAdbPort(this.pkgName);

        try {
            System.out.println("Connect to " + pkgName + " @LocalPort:" + port + " @RemotePort:" + port);
            Runtime.getRuntime().exec(String.format("adb forward tcp:%d tcp:%d", port, port));
        } catch (IOException e) {
            e.printStackTrace();
